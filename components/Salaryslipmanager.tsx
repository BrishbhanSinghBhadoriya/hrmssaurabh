...
const SalarySlipManager: React.FC = () => {
    ...
    const fetchEmployees = async (): Promise<void> => {
        try {
            setLoading(true);
            // We'll try /api/hr/getEmployees first (as per latest backend updates)
            // if that fails, we fallback to /hr/getEmployees (legacy working endpoint)
            let response;
            try {
                response = await api.get('/api/hr/getEmployees');
            } catch (err) {
                console.warn('Attempting legacy endpoint /hr/getEmployees after /api/hr/getEmployees failed');
            ...
            if (employeesData && Array.isArray(employeesData)) {
                ...
                console.error('Invalid employees data structure:', response.data);
                alert('Backend returned invalid data structure. Check console for details.');
            }
        } catch (error) {
            console.error('Final Error fetching employees:', error);
            alert('Failed to fetch employees. Please check if backend is running or your role has permissions.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDownloadSalarySlip = async (): Promise<void> => {
        ...
        try {
            ...
            try {
                ...
                // Fallback to previous potential paths
                try {
                    response = await api.get(
                        `/api/salary-slip/${selectedEmployee.employeeId}/${selectedMonth}/${selectedYear}`,
                        { responseType: 'blob' }
